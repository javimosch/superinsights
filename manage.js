...
async function deployDomain() {
  ...
  console.log(
    `🌐 Your API should now be accessible at: https://${config.PUBLISHED_DOMAIN}`,
  );
  console.log(
    `\ncURL to test the API:\ncurl https://${config.PUBLISHED_DOMAIN}/health`,
  );
  ...
  while (true) {
    ...
    try {
      const result = execSync(
        `curl -s https://${config.PUBLISHED_DOMAIN}/health`,
        { encoding: "utf8" },
      );
      if (result.includes("ok")) {
        console.log(
          `✅ API is now accessible at: https://${config.PUBLISHED_DOMAIN}`,
        );
        break;
      }
    } catch (e) {
      ...
      try {
        const statusCode = execSync(
          `curl -s -o /dev/null -w "%{http_code}" https://${config.PUBLISHED_DOMAIN}`,
          { encoding: "utf8" },
        );
...
async function createProxyFile() {
  ...
  routers:
    ${traefikServiceName}:
      ...
      rule: Host(\`${config.PUBLISHED_DOMAIN}\`)
      tls:
...
async function buildImage() {
  ...
  if (composeFiles.length === 0) {
    console.error("❌ Error: No compose files found in the current directory.");
    return;
  }
  ...
  const imageChoice = await prompt("Select an image to push (enter number): ");
